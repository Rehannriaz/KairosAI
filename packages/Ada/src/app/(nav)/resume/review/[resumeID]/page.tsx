...
export default function ResumeReview({ params }: PageProps) {
  ...
  useEffect(() => {
    const fetchResumeData = async () => {
      if (params.resumeID) {
        try {
          setIsLoading(true);
          const resumeData = await resumeServiceInstance.getResumeById(
  ...
  const handleOptimize = async (
    ...
    try {
      ...
      const response = await resumeServiceInstance.optimizeResumeText(
        textToOptimize,
        section
      );
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleContentChange('location', e.target.value)
                  }
                />
              </div>
              <div>
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              onChange={(e) =>
                handleContentChange('professional_summary', e.target.value)
              }
          ...
          <div className="space-y-4">
            ...
            {resume.employment_history.map((job, index) => (
              <div
                key={index}
                className="space-y-2 pr-4 pb-4 pl-4 pt-10 border rounded-md relative"
              >
                <Button
                  variant="ghost"
                  size="icon"
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() =>
                    handleOptimize('employment_history', job.description, index)
                  }
                  className="h-8 px-2 text-xs"
                  disabled={isOptimizing}
                >
                ...
                {optimizationResults?.jobOptimizations &&
                  optimizationResults.jobOptimizations[index] && (
                    <div className="mt-2">
                      <p className="text-sm font-medium">Added keywords:</p>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {optimizationResults.jobOptimizations[
                          index
                        ].added_keywords.map((keyword, keywordIndex) => (
